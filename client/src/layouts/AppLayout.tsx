      ...
      <main className="flex-1 flex flex-col relative min-w-0 bg-[#050505]">
        ...
        {!location.pathname.includes('/chat/') && (
          <nav className="md:hidden h-20 bg-black/90 backdrop-blur border-t border-gray-900 flex justify-around items-center px-2 z-40 fixed bottom-0 left-0 right-0 pb-safe">
            ...
            <button onClick={() => navigate('/notifications')} className={`p-2 flex flex-col items-center gap-1 ${isActive('/notifications') ? 'text-neon' : 'text-gray-600'}`}>
              <div className={`p-1 rounded-xl ${isActive('/notifications') ? 'bg-neon/10' : ''} relative`}>
                <Bell className="w-6 h-6" />
                {unreadNotifs > 0 && <span className="absolute top-1 right-1 w-2 h-2 bg-neon rounded-full animate-pulse" />}
              </div>
